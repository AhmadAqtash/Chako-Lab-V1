...
export default function FloatingContact() {
  ...
  // When the PDP's sticky ATC bar slides in (mobile), it lands right on the
  // lower icons — hop the stack up out of its way, bottom icon first, with a
  // springy overshoot. The lift is max-md only: the bar itself is md:hidden,
  // but its IntersectionObserver still fires on desktop.
  const [lifted, setLifted] = useState(false);
  useEffect(() => {
    const onSticky = (e: Event) =>
      setLifted((e as CustomEvent<{ visible?: boolean }>).detail?.visible === true);
    window.addEventListener('chako:sticky-atc', onSticky);
    return () => window.removeEventListener('chako:sticky-atc', onSticky);
  }, []);

  // Bottom icon leads on the way up; top icon leads on the way back down —
  // reads as one wave in both directions. Reduced-motion still moves
  // (overlap is a usability problem), just without the transition.
  const hop = (indexFromBottom: number) =>
    ({
      className: cn(
